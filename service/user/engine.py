...
# Paths
source_dir = r'C:\Users\Jagasabarivel_K\AppData\Local\Google\Chrome\User Data'
target_dir = r'C:\tmp\chrome-data\watsapp-session'

class SBMGoogleSheet:
    # Path to the service account key file
    SERVICE_ACCOUNT_FILE = r'C:\Users\Jagasabarivel_K\OneDrive - Dell Technologies\Documents\Learning\Broadcast Service\BE\sbm-bcast\config\g.json'
    # Define the scope
    ...
    def __init__(self, sheet_url):
        ...
        self.creds = Credentials.from_service_account_file(
            SBMGoogleSheet.SERVICE_ACCOUNT_FILE,
...
class SBMWatsApp:
    ...
    def send_message(self, conn, **data):
        ...
        msg_status = "success"
        try:
            self.browser = self.open_browser(True)
            sheet = SBMGoogleSheet(data.get("google_sheet_url"))
            ...
            for customer in sheet.fetch_all_records()[1:]:
                ...
                try:
                    ...
                    inp_xpath = self.get_chat_box()
                    
                    print("before input_box ")
                    input_box = WebDriverWait(self.browser, 10).until(EC.presence_of_element_located((By.XPATH, inp_xpath)))
                    ...
                    print("Message Sent")
                    time.sleep(10)
                    models.update_message_status(conn, wphone, format_msg)

                except Exception as _e:
                    models.update_message_status(conn, wphone, format_msg, "failed")
    ...
    def copy_to_destination(self):
        # 2. Copy files from source directory to target directory
      ...
      try:
        ...
        for job in jobs:
          with models.get_msg_db() as conn:
              ...
              print("frequence ", type(frequency))
              print("status ", type(status))
              if (
                  (updated_at <= datetime.now() and status=="queued") or
